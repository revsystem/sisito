...
- Pi は現在 `heads/stats_date_range_all_charts` を checkout した状態。未マージブランチのため、PR を作成してマージした上で master へ戻すか、検証完了として先に master へ戻すかの判断が必要
- PR は未作成。Issue も未起票
- `bin/deploy.sh` を通した正規のデプロイは未実施（gem・アセット変更が無いため今回は不要だった）
