...
12. **`etc` slice double-counts in all six pie/donut chart blocks**: the shared idiom is `<collection>.slice(0, 10).push(['etc', <values>.slice(3..-1).try(:sum)])` — the individual slices show the top 10 while `etc` sums entries 4 onward, so entries 4–10 are counted twice and the chart total exceeds the real count. Present in `_recent_bounced.html.erb` (2×) and `_unique_recipient_bounced.html.erb` (3×), which build `<values>` from a Hash via `.values`, and in `_bounced_by_type.html.erb` (1×), which works on a sorted Array and uses `.map(&:last)` instead — same bug, different accessor. When cross-checking a chart against direct SQL, compare individual slices, not the donut's centre total.
