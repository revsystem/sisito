...
### `bin/sync-and-ingest.sh` + `update-sisto-db-safe.rb` (Pi-only, not tracked in git)
The Pi's actual production ingestion path, running unattended since some point after 2026-07-21 — despite "Data Integration" above describing ingestion as manual. Neither file is checked into this repo; both exist only on the Pi (`/home/pi/sisito/bin/sync-and-ingest.sh`, `/home/pi/sisito/update-sisto-db-safe.rb`) and must be read there (`ssh pi@192.168.1.12`) when debugging, since there is no local copy to grep.

`bin/sync-and-ingest.sh` runs from cron (`0 20 * * *`), `flock`-guarded against overlapping runs. It `rsync`s from a remote mail-archive host (`bounce-sync@ml.msc-dance.jp`, port 2222, `rrsync -ro` restricted) into `tmp_mail/incoming/`, then invokes `update-sisto-db-safe.rb` against that directory. The remote is read-only from this end, so it never deletes its files — every sync re-pulls everything the mail server still has.

`update-sisto-db-safe.rb` handles that redundancy with a `tmp_mail/processed.log` (one filename per line): already-logged files are deleted locally without touching the DB, so re-syncing the same file is a no-op. New files are parsed with `Sisimai.rise` in an isolated `Dir.mktmpdir` copy (the original is never moved into the tmpdir, so its auto-cleanup can't take the source file with it), inserted inside a `BEGIN`/`COMMIT` transaction per file, and only added to `processed.log` (then deleted) strictly after that commit succeeds — a crash between commit and log-write just means a harmless duplicate-insert retry next run, never silent data loss. Rows use the same `FROM_UNIXTIME(...)` / `SHA1(recipient)` insert shape as `update-sisto-db.rb` above. Failed/unparseable files move to sibling `tmp_mail/failed/` / `tmp_mail/unrecognized/` directories instead of blocking the rest of the batch.
...
13. **Cron jobs on the Pi must invoke `ruby` via mise's absolute path, not bare `ruby` or `mise exec --`**: cron's minimal `PATH` (`/usr/bin:/bin`) has neither the project's mise-pinned Ruby 3.4.9 nor the `mise` binary itself (`/home/pi/.local/bin/mise`, only on `PATH` via the interactive shell profile). A bare `ruby` call falls through to the system Ruby (3.2.3) with its own separate, essentially abandoned `vendor/bundle/ruby/3.2.0` tree that nothing keeps in sync with `Gemfile.lock` — any Gemfile.lock-changing merge (a gem version bump, a CVE fix, …) silently strands it further. Unlike a long-running server process, which keeps working on its already-loaded gems until it's next restarted, a cron script re-execs fresh on every scheduled run, so it starts failing on the very next run after such a merge — with no visible symptom until someone happens to check. This is exactly what broke `bin/sync-and-ingest.sh`'s automated bounce ingestion for four days (2026-08-15 through 2026-08-18) after the `json` 2.21.2 CVE bump (#41): every run hit `Bundler::GemNotFound: Could not find json-2.21.2`, while the `rsync` step before it kept succeeding, so mail piled up unprocessed in `tmp_mail/incoming/` until the dashboard was checked and found missing recent days. Fixed by calling the absolute path directly: `/home/pi/.local/bin/mise exec -- ruby update-sisto-db-safe.rb ...`. Any future cron-invoked script on the Pi needs the same absolute-path treatment, not just this one.
