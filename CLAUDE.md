...
6. **Branching**: `master` is the default/production branch. Earlier history used long-lived version branches (e.g. `heads/Rails_v7.2.3.1`; the oldest, `Rails_v5.1.0`, survives only as the tag `archive_Rails_v5.1.0_branch_tip`) — that pattern is gone. Current practice is a short-lived `heads/<description>` branch per change, squash-merged into `master`.
